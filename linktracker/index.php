<?php
  ...
  */
  ...
  if(isset($_SERVER['HTTP_REFERER'])) { $slt_regerral = $_SERVER['HTTP_REFERER']; } else { }
  /*
    ip-api.com API part..
  */
  ...
  */
  ...
  if($track == '') {
    ...
    $get_tracking_query = "SELECT * FROM links WHERE slt_link_trackingid = '$track'";
    if (!$get_tracking = mysqli_query($conn, $get_tracking_query)) {
    ...
    while($row = mysqli_fetch_assoc($get_tracking)) {
      ...
      $slt_link_trackingid = $row["slt_link_trackingid"];
        ...
        $get_visitor_query = "SELECT * FROM tracking WHERE slt_tracking_ipaddr = '$slt_ipaddr' AND slt_tracking_trackid = '$slt_link_trackingid'";
        if (!$get_visitor = mysqli_query($conn, $get_visitor_query)) {
          exit(mysqli_error($conn));
        } 
          $get_visitor_amount = mysqli_num_rows($get_visitor);
            if($get_visitor_amount > 0) {
              ...
              */
              ...
              update:
              /* 
                Bot blocking part.. This should either be made into an array or even better if a mysql table for this one
              */
              ...
              $insertdata_tracking_query = "INSERT INTO tracking 
              ...
              if (!$insertdata_tracking = mysqli_query($conn, $insertdata_tracking_query)) {
                exit(mysqli_error($conn));
              }  
              //update total visits row.
              $insertdata_links_query = "UPDATE links SET slt_link_total = slt_link_total + 1 WHERE slt_link_trackingid = '$slt_link_trackingid'";
              if (!$insertdata_links = mysqli_query($conn, $insertdata_links_query)) {
                exit(mysqli_error($conn));
              }  
...
if (!$check = mysqli_query($conn, $check_query)) {
  exit(mysqli_error($conn));
}  
while($info = mysqli_fetch_array( $check )) {     
 ...
 if (isset($_POST['submit'])) { 
  ...
  $check_query = "SELECT * FROM users WHERE slt_user_email = '".$_POST['email']."'";
  if (!$check = mysqli_query($conn, $check_query)) {
    exit(mysqli_error($conn));
  } 
...
<center>
 ...
 <!-- Bookmark widget. -->
  <a id="bookmarklet" href="javascript:void(window.open('http://x-url.eu/?url='+encodeURIComponent(document.URL)))" onclick="return false;">Shorten</a>
   ...
   <style>
   #bookmarklet { 
      height: 30px; top: 0px; position: absolute; width: 100%; background: transparent; z-index: 10;
      font-size: 0; font-weight: bold; text-align: center; text-decoration: none;
      cursor: move;
    }
    #bookmarklet_bg { 
      height: 30px; top: 0px; position: absolute; width: 100%; background: #34495e; border-radius: 6px; 
      font-size: 16px; color: #fff; font-weight: bold; text-align: center; text-decoration: none;
    }
   </style>
